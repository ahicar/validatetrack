...
const { validateDeviation } = require('../utils/validators');

const client = new DynamoDBClient({});
const docClient = DynamoDBDocumentClient.from(client);
const TABLE_NAME = process.env.DEVIATIONS_TABLE;
...
async function getOne(id) {
    ...
    if (!result.Item) return response(404, { message: 'Deviation not found' });
    return response(200, result.Item);
}

async function create(data) {
    const { isValid, errors } = validateDeviation(data);
...
async function update(id, data) {
    ...
    if (!existing.Item) return response(404, { message: 'Deviation not found' });
    ...
    const updated = {
        ...
        description: data.description,
        severity: data.severity,
        status: data.status || existing.Item.status,
